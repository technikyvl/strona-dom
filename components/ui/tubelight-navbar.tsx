...
export function NavBar({ items, className }: NavBarProps) {
  ...
  useEffect(() => {
    const onScroll = () => {
      ...
      setHidden(goingDown && y > 80) // hide after slight scroll
  ...
  return (
    <motion.div
      animate={{ y: hidden ? (isMobile ? 96 : -96) : 0, opacity: hidden ? 0.9 : 1 }}
    ...
    >
      <div className="flex items-center gap-3 bg-white text-foreground border border-border py-1 px-1 rounded-full shadow-lg">
        {items.map((item) => {
